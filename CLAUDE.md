...
**Repeat avoidance** — `getSeenIds`/`addSeenIds` in `app.js` track up to `SEEN_MAX` (currently 500, ~50 games' worth at the default 10 questions/game) most-recently-played question IDs per player in `localStorage` key `disney_seen_{userId}` — **per-device, not synced to Firestore**, like the other `disney_*` localStorage keys, so the same player switching devices won't carry their seen-history over. `SEEN_MAX` was 100 originally, raised to 300 on 2026-06-21 (commit `3339707`), then to 500 on 2026-07-29 for more freshness as the question bank grew — if this number is ever changed again, update it here too.

- **Recording** (`addSeenIds`, called from `endGame()` unconditionally, and from the mid-game exit handler with ≥1 answered question) happens for **both** regular games and daily challenges — completing either marks those question IDs as seen.
- **Selection filtering** only happens for regular Play Game / Rematch: the pool (post difficulty/category filter) is split into fresh (unseen) vs. all; if enough fresh questions exist to fill the requested count, only fresh ones are used — otherwise **the exclusion is dropped entirely** and the full filtered pool (including recently-seen questions) is used instead, rather than partially topping off with repeats. A game with a narrow filter (single category + difficulty, well under ~500 matching questions) can serve repeats sooner than the default all-categories setting would.
- **The daily challenge's own question selection (`getDailyQuestions`) never consults seen-ids at all**, in either direction — by design, since the day's 10 questions must be identical for every player regardless of what any one player has personally seen. This also means **there is currently no cross-day duplicate protection for the daily challenge** — `getDailyQuestions` reshuffles the *entire* question pool independently per calendar day (date-seeded, not incremental), so the same question can be selected for two different days' dailies purely by chance, with nothing (not seen-ids, not the `dailies` Firestore history) preventing it. Confirmed 2026-07-29 after a user reported seeing a repeat (the Galactic Starcruiser/"Halcyon" hotel question, #352) within roughly a 100-question span — plausible if that span included multiple daily challenges, since daily-to-daily is the one path with zero repeat protection. Regular-game-to-regular-game and daily-to-regular-game repeats *are* protected by the seen-ids mechanism above; daily-to-daily is not. Fixing this would need the daily-selection code to also exclude recent days' pinned IDs (available in the `dailies` Firestore collection) — not yet implemented, flagged here as a known gap rather than done speculatively.
