...
**Repeat avoidance** — `getSeenIds`/`addSeenIds` in `app.js` track up to `SEEN_MAX` (currently 300, ~30 games' worth at the default 10 questions/game) most-recently-played question IDs per player in `localStorage` key `disney_seen_{userId}` — **per-device, not synced to Firestore**, like the other `disney_*` localStorage keys, so the same player switching devices won't carry their seen-history over (this is also why a repeat can still slip through if the same player uses two different devices — the two seen-lists never merge). `SEEN_MAX` was 100 originally, raised to 300 on 2026-06-21 (commit `3339707`) for more freshness as the question bank grew; briefly tried 500 on 2026-07-29 same-day, then reverted back to 300.

- **Recording** (`addSeenIds`) happens for both regular games and daily challenges, on: full completion (`endGame()`, unconditional) and mid-game exit with ≥1 answered question — including a partial daily exit, fixed 2026-07-29 (previously only regular-game mid-exit recorded seen-ids; exiting a daily partway through saved progress for resuming but never marked the already-answered questions as seen, so one of them could still show up in a regular game played before the player went back to finish that day's daily — a plausible explanation for a reported repeat).
- **Selection filtering** only happens for regular Play Game / Rematch: the pool (post difficulty/category filter) is split into fresh (unseen) vs. all; if enough fresh questions exist to fill the requested count, only fresh ones are used — otherwise **the exclusion is dropped entirely** and the full filtered pool (including recently-seen questions) is used instead, rather than partially topping off with repeats. A game with a narrow filter (single category + difficulty, well under ~300 matching questions) can serve repeats sooner than the default all-categories setting would.
- **The daily challenge's own question selection (`getDailyQuestions`) never consults seen-ids at all**, in either direction — by design, since the day's 10 questions must be identical for every player regardless of what any one player has personally seen, and seen-ids are local-only anyway so there's no single source of truth to check even if it wanted to. This also means **there is currently no cross-day duplicate protection for the daily challenge** — `getDailyQuestions` reshuffles the *entire* question pool independently per calendar day (date-seeded, not incremental), so the same question can be selected for two different days' dailies purely by chance. Known gap, not yet fixed — see the 2026-07-29 discussion in git history/chat for the design options considered (per-user seen-ids synced to Firestore vs. a single shared "recently used" log consulted by both daily generation and regular-game selection); a shared log was the leaning candidate since this is a small fixed household of players, not a per-account personalization problem.
