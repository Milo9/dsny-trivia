...
**Repeat avoidance** (non-daily games only — the daily challenge is pinned/shared per calendar day and deliberately doesn't use this): `getSeenIds`/`addSeenIds` in `app.js` track up to `SEEN_MAX` (currently 300, ~30 games' worth at the default 10 questions/game) most-recently-played question IDs per player in `localStorage` key `disney_seen_{userId}` — **per-device, not synced to Firestore**, like the other `disney_*` localStorage keys, so the same player switching devices won't carry their seen-history over. `SEEN_MAX` was raised from its original 100 to 300 on 2026-06-21 (commit `3339707`) for more freshness as the question bank grew — if this number is ever changed again, update it here too. On starting a game or hitting Rematch, the pool (post difficulty/category filter) is split into fresh (unseen) vs. all; if enough fresh questions exist to fill the requested count, only fresh ones are used — otherwise **the exclusion is dropped entirely** and the full filtered pool (including recently-seen questions) is used instead, rather than partially topping off with repeats. This means a game with a narrow filter (e.g. a single category + difficulty, well under ~300 matching questions) can serve repeats sooner than the default all-categories setting would. Seen-ids are recorded after a completed game (`endGame()`) or a mid-game exit with at least one answered question — an unsaved/failed game leaves seen-ids untouched so its questions can come back around.
