...
| Script | Purpose |
|---|---|
| `recon.py <regex>` | `id + question` dump for a keyword/film — a token-cheap first pass over full Grep hits during the mandatory pre-draft recon scan. Hides answers by default (`--show-answer` adds only the correct answer, never distractors) — good for "has this stem/angle been asked," not a substitute for a real Grep hit when judging whether a specific fact+answer is already covered. `--all-fields` also searches wrong answers. |
